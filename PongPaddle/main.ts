namespace PongPaddle{
    import ƒ = FudgeCore; //import FudgeCore as ƒ

    window.addEventListener("load", hndload); //add hndload to load Event, happening when page is loading
    let viewport: ƒ.Viewport; //refenence

    let ball: ƒ.Node = new ƒ.Node("Ball"); //reference to Ball node
    let paddleLeft: ƒ.Node = new ƒ.Node("PaddleLeft"); //reference to Left Paddle Node
    let paddleRight: ƒ.Node = new ƒ.Node("PaddleRight"); //reference to Right Paddle Node

    //Loading Handler
    function hndload(){
        const canvas: HTMLCanvasElement= document.querySelector("canvas"); //reference to Canvas Element
        ƒ.RenderManager.initialize(); //Initialize RenderingContext of WebGL
        ƒ.Debug.log(canvas); //use of Fudge Debugger

        //pong scene
        let pong: ƒ.Node= createPong(); //the root Node of this scene

        //Camera
        let cmpCamera: ƒ.ComponentCamera= new ƒ.ComponentCamera(); //the Camera initialization
    ...
    function createPong(): ƒ.Node {
        let pong: ƒ.Node= new ƒ.Node("Pong");   //init Pong Node

        let mtrSolidWhite: ƒ.Material= new ƒ.Material("SolidWhite", ƒ.ShaderUniColor, new ƒ.CoatColored(ƒ.Color.CSS("WHITE"))); //white solid material

        let meshQuad: ƒ.MeshQuad= new ƒ.MeshQuad(); //Quad Mesh
